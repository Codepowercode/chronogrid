...
class SalesProductsExport implements FromCollection
{
    ...
    {
        $orders = Order::query()->with(['product'])->where('seller_id', $this->id);

        if ($this->type == 'sales_marketplace'){
            $orders = $orders->where('auction', 0);
        }elseif ($this->type == 'sales_auction'){
            $orders = $orders->where('auction', 1);
        }
